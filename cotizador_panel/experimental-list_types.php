...
$apartments = getApartments($_GET['pID']);
$types = getApartmentsTypes($_GET['pID']);
$programs = getApartmentsPrograms($_GET['pID']);
        ...
        <table class="wp-list-table widefat fixed custom-list">
            ...
            <tbody>
                <?php foreach($apartments as $apartment){ ?>
                	<tr>
	                	<td align="center"><?= $apartment->floor ?></td>
	                	<td align="center"><?= $apartment->number ?></td>
	                	<td><?= $apartment->program_name ?></td>
	                	<td><?= $apartment->type_name ?></td>
	                	<td><?= number_format($apartment->price, 0, ',', '.') ?></td>
	                	<td><?= ucwords(strtolower($apartment->status)) ?></td>
	                	<td align="center"><?= $apartment->orientation ?></td>
	                	<td><?= $apartment->rooms ?> pieza(s)</td>
	                	<td><?= $apartment->bathrooms ?> baño(s)</td>
	                	<td><?= $apartment->kitchen_type == 'C' ? 'Cocina' : 'Kitchen'; ?></td>
	                	<td><?= number_format($apartment->m2_floor, 2, ',', '.') ?> m2</td>
	                	<td><?= number_format($apartment->m2_terrace, 2, ',', '.') ?> m2</td>
	                	<td align="right">
	                        <a href="?page=cotizador&action=view_type&tID=<?=$type->id?>&pID=<?=$_GET['pID']?>">
	                            <button data-id="<?=$type->id?>" class="button-primary toggle-control">Editar</button>
	                        </a>
	                        <a href="?page=cotizador&action=list_types&deleteID=<?=$type->id?>&pID=<?=$_GET['pID']?>&noheader=true">
	                            <button data-id="<?=$type->id?>" class="button toggle-control">Eliminar</button>
	                        </a>
	                    </td>
                    </tr>
                <?php } ?>
        ...
        <table class="wp-list-table widefat fixed custom-list">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: right;">UF <?=$quote->optional_warehouse?></td>                    
                    <td style="text-align: right;">UF <?=$quote->optional_parking?></td>                                        
